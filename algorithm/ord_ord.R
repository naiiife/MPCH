cppFunction('
  List ord_ord(IntegerVector Y1, IntegerVector Y2,
  ...
  NumericVector delta_2 = res2["delta"];
  ...
  int L2 = delta_2.size() + 1;
  NumericVector dt2 (L2+1);
  dt2[0] = -999.0;
  dt2[L2] = 999.0;
      ...
      for (int b1=0; b1<ngrid; b1++){
        ...
        for (int b2=0; b2<ngrid; b2++){
          ...
          for (int b=0; b<ngrid; b++){
            ...
            double linear2 = dt2[Y2i+1]-sum(X2i*alpha_2)-theta_2*e-eps2;
            double linear2l = linear2-dt2[Y2i+1]+dt2[Y2i];
            double Plinear2 = erf(linear2/1.4142136)/2.0- erf(linear2l/1.4142136)/2.0;
            double loglik1 = log(Plinear1);
            double loglik2 = log(Plinear2);
  ...
  NumericVector dl_eta1 (p1+1+L1);
  NumericVector dl_eta2 (p2+1+L2);
  NumericVector dl_gamma_eta1 (p1+1+L1);
  NumericVector dl_gamma_eta2 (p2+1+L2);
  NumericVector dl2_gamma_eta1 (p1+1+L1);
  NumericVector dl2_gamma_eta2 (p2+1+L2);
  NumericVector dl_gamma_dl_eta1 (p1+1+L1);
  NumericVector dl_gamma_dl_eta2 (p2+1+L2);
  for (int i=0; i<N; i++){
    int Y1i = Y1[i];
    int Y2i = Y2[i];
    NumericVector X1i = X1(i,_);
    ...
    NumericVector num7 (1+L2+p2);
    NumericVector num8 (1+L2+p2);
    NumericVector num9 (1+L2+p2);
    for (int b1=0; b1<ngrid; b1++){
      double b_eps1 = 1.4142136 * b_list[b1];
      double w_eps1 = w_list[b1];
      for (int b2=0; b2<ngrid; b2++){
        ...
        for (int b=0; b<ngrid; b++){
          ...
          for (int k=0; k<p1; k++){
            double d3 = -(dnorm1-dnorm1l)/Plinear1*X1i[k];
            if(Plinear1==0) {d3=0;}
          ...
          double dnorm2 = exp(-linear2*linear2/2.0)/2.506628;
          double dnorm2l = exp(-linear2l*linear2l/2.0)/2.506628;
          de0 = -(dnorm2-dnorm2l)/Plinear2*e;
          ...
          for (int k=0; k<p2; k++){
            double d3 = -(dnorm2-dnorm2l)/Plinear2*X2i[k];
            if(Plinear2==0) {d3=0;}
            num9[k+2] = num9[k+2] + wdens * d3;
            num7[k+2] = num7[k+2] + wdens * d3*dldg;
          }
          for (int l=0; l<L2-1; l++){
            double d3 = 0;
            if(Y2i==l) {d3 = dnorm2/Plinear2;}
            if(Y2i==l+1) {d3 = -dnorm2l/Plinear2;}
            if(Plinear2==0) {d3=0;}
            num9[2+p2+l] = num9[2+p2+l] + wdens * d3;
            num7[2+p2+l] = num7[2+p2+l] + wdens * d3*dldg;
          }
  ...
  double sigma_2 = sqrt(theta_2*theta_2+gamma_2+1.0);
  ...
  NumericVector Drho_2 (p2+2+L2);
  NumericVector Drho12_1 (p1+2+L1);
  NumericVector Drho12_2 (p2+2+L2);
  NumericVector Dzeta_1 (p1+2+L1);
  NumericVector Dzeta_2 (p2+2+L2);
  NumericVector Dzeta12_1 (p1+2+L1);
  NumericVector Dzeta12_2 (p2+2+L2);
