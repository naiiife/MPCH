cppFunction('
  List con_ord(NumericVector Y1, IntegerVector Y2,
  ...
  int L = delta_2.size() + 1;
  NumericVector dt (L+1);
  dt[0] = -999.0;
  dt[L] = 999.0;
  for (int l=0; l<L-1; l++){
  dt[l+1] = delta_2[l];
  ...
  while(tol>0.001){
      ...
      for (int b1=0; b1<ngrid; b1++){
        ...
        for (int b2=0; b2<ngrid; b2++){
          ...
          for (int b=0; b<ngrid; b++){
            ...
            double linear2 = dt[Y2i+1]-sum(X2i*alpha_2)-theta_2*e-eps2;
            double linearl = linear2-dt[Y2i+1]+dt[Y2i];
            double Plinear_2 = erf(linear2/1.4142136);
            double Plinear_l = erf(linearl/1.4142136);
            double Plinear = (Plinear_2 - Plinear_l)/2.0;
            double loglik1 = -linear1*linear1/2.0/sigmau21 - log(sigmau21)/2.0;
            double loglik2 = log(Plinear);
    ...
    maxlk = max(lk);
    tol = sqrt((gamma120-gamma12)*(gamma120-gamma12));
  ...
  NumericVector dl_eta2 (p2+1+L);
  NumericVector dl_gamma_eta1 (p1+3);
  NumericVector dl_gamma_eta2 (p2+1+L);
  NumericVector dl2_gamma_eta1 (p1+3);
  NumericVector dl2_gamma_eta2 (p2+1+L);
  NumericVector dl_gamma_dl_eta1 (p1+3);
  NumericVector dl_gamma_dl_eta2 (p2+1+L);
  for (int i=0; i<N; i++){
    double Y1i = Y1[i];
    int Y2i = Y2[i];
    NumericVector X1i = X1(i,_);
    ...
    NumericVector num7 (1+L+p2);
    NumericVector num8 (1+L+p2);
    NumericVector num9 (1+L+p2);
    for (int b1=0; b1<ngrid; b1++){
      double b_eps1 = 1.4142136 * b_list[b1];
      double w_eps1 = w_list[b1];
      for (int b2=0; b2<ngrid; b2++){
        ...
        for (int b=0; b<ngrid; b++){
          ...
          for (int k=0; k<p1; k++){
            double d3 = linear1*X1i[k]/sigmau21;
          ...
          double dnorml = exp(-linearl*linearl/2.0)/2.506628;
          de0 = -(dnorm2-dnorml)/Plinear*e;
          if(Plinear==0) {de0 = 0;}
          ...
          for (int k=0; k<p2; k++){
            double d3 = -(dnorm2-dnorml)/Plinear*X2i[k];
            if(Plinear==0) {d3 = 0;}
          ...
          for (int l=0; l<L-1; l++){
            double d3 = 0;
            if(Y2i==l) {d3 = dnorm2/Plinear;}
            if(Y2i==l+1) {d3 = -dnorml/Plinear;}
            if(Plinear==0) {d3 = 0;}
  ...
  NumericVector Drho_2 (p2+2+L);
  NumericVector Drho12_1 (p1+4);
  NumericVector Drho12_2 (p2+2+L);
  NumericVector Dzeta_1 (p1+4);
  NumericVector Dzeta_2 (p2+2+L);
  NumericVector Dzeta12_1 (p1+4);
  NumericVector Dzeta12_2 (p2+2+L);
